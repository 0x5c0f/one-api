...
func postConsumeQuota(ctx context.Context, usage *relaymodel.Usage, meta *meta.Meta, textRequest *relaymodel.GeneralOpenAIRequest, ratio float64, preConsumedQuota int64, modelRatio float64, groupRatio float64) {
	...
	if len(textRequest.Messages) > 0 {
		if msg, ok := textRequest.Messages[len(textRequest.Messages)-1].Content.(string); ok {
			lastMsg = msg
			// 此处用于截断 fastgpt 带过来的其他内容
			msgsplit := strings.Split(lastMsg, ">>>>")

			if len(msgsplit) > 1 {
				lastMsg = msgsplit[len(msgsplit)-1]
			}
	...
	logContent := fmt.Sprintf("模型倍率 %.2f，分组倍率 %.2f，补全倍率 %.2f, Input: %s", modelRatio, groupRatio, completionRatio, lastMsg)
